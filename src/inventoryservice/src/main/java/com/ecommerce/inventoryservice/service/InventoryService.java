...
public class InventoryService {
    ...
    public SoftReserveResponseDTO softReserve(SoftReserveRequestDTO request) {
        ...
        }, request.getProductId(), "softReserve");
    }
    ...
    public void hardReserveInventory(Long productId, int quantity, Long orderId) {
            ...
            try {
                ...
                if (newReservedQuantityRedis == null || newReservedQuantityRedis < 0) {
                    ...
                    if (inventoryOptional.isPresent()) {
                        Inventory inventory = inventoryOptional.get();
                        redisTemplate.opsForValue().set(String.valueOf(inventory.getReservedQuantity()), inventory.getReservedQuantity());
                        ...
                        if (newReservedQuantityRedis == null || newReservedQuantityRedis < 0) {
                            ...
                            return null; // Cannot proceed with hard reservation
                        }
                    } else {
                ...
                if (newReservedQuantity < 0) {
                    ...
                    return null; // Prevent negative reserved quantity in DB
                }
        ...
        }, productId, "hardReserveInventory");
    }
    ...
    public void rollbackSoftReservation(Long productId, int quantity, Long orderId) {
                ...
                if (newAvailableQuantityRedis == null || newReservedQuantityRedis == null || newReservedQuantityRedis < 0) {
                    ...
                    if (inventoryOptional.isPresent()) {
                        ...
                        if (newAvailableQuantityRedis == null || newReservedQuantityRedis == null || newReservedQuantityRedis < 0) {
                            ...
                            return null; // Cannot proceed with rollback
                        }
                    } else {
        ...
        }, productId, "rollbackSoftReservation");
    }
    ...
    private <T> T executeWithRetry(Supplier<T> operation, Long productId, String methodName) {
        int attempts = 0;
        while (true) {
            try {
                return operation.get();
            } catch (ObjectOptimisticLockingFailureException ex) {
                log.warn("ObjectOptimisticLockingFailureException occurred during {} for product {}. Retrying... Attempts: {}",
                        methodName, productId, attempts + 1);
                if (++attempts >= MAX_RETRIES) {
                    log.error("Max retries reached for {} and product {}. Aborting.", methodName, productId);
                    throw ex; // Re-throw the exception if max retries reached
                }
                try {
                    Thread.sleep((attempts * RETRY_DELAY_MS)); // Exponential backoff
                } catch (InterruptedException e) {
                    Thread.currentThread().interrupt();
                    log.error("Thread interrupted during retry backoff for {} and product {}.", methodName, productId, e);
                    throw new RuntimeException("Thread interrupted during retry", e);
                }
             } catch (OptimisticLockingFailureException ex) {
                log.warn("OptimisticLockingFailureException occurred during {} for product {}. Retrying... Attempts: {}",
                        methodName, productId, attempts + 1);
                if (++attempts >= MAX_RETRIES) {
                    log.error("Max retries reached for {} and product {}. Aborting.", methodName, productId);
                    throw ex; // Re-throw the exception if max retries reached
