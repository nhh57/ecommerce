...
- [x] **Xác minh và sử dụng `@Version` trong `Inventory` entity:** (Đã hoàn thành)
...
- [x] **Xử lý `OptimisticLockingFailureException`:** (Đã hoàn thành)
...
- [x] **Sử dụng các thao tác atomic trên Redis:** (Đã hoàn thành)
...
- [x] **Xử lý các xung đột CAS:** (Đã hoàn thành)
    - **Ghi chú:** Triển khai logic để bắt các lỗi CAS và thực hiện retry.

## Tiến độ

*   **Optimistic Locking (DB):** [x]
*   **Compare-and-Swap (CAS) (Redis):** [x]
